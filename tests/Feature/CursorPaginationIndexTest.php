...
class CursorPaginationIndexTest extends TestCase
{
    ...
    public function test_cursor_list_queries_use_scope_and_order_indexes_without_temporary_sorting(): void
    {
        ...
        $this->assertIndexedPlan(
            'SELECT * FROM product_reviews WHERE seller_id = ? ORDER BY created_at DESC, id DESC LIMIT 5',
            [1],
        ...
        $this->assertIndexedPlan(
            'SELECT * FROM fulfillment_messages WHERE fulfillment_id = ? ORDER BY created_at DESC, id DESC LIMIT 10',
            [1],
            'messages_thread_cursor_index',
        );
    }
